...
const URL_STATUS = "intern_api_cryptos_cryptocom_status";
const URL_CONNECT = "intern_api_cryptos_cryptocom_connect";
const URL_SYNC = "intern_api_cryptos_cryptocom_sync";
const URL_DISCONNECT = "intern_api_cryptos_cryptocom_disconnect";
...
export function CryptocomConnect ({ onSynced }) {
	...
	const fetchStatus = () => {
		axios.get(Routing.generate(URL_STATUS))
			...
			.catch((error) => Formulaire.displayErrors(null, error, "Impossible de récupérer le statut Crypto.com Exchange."))
		;
	}
	...
	const handleConnect = () => {
		...
		axios({ method: "POST", url: Routing.generate(URL_CONNECT), data: { apiKey, apiSecret } })
			.then(() => {
				Toastr.toast('info', "Compte Crypto.com Exchange connecté.");
				setApiKey("");
				setApiSecret("");
				fetchStatus();
			})
	...
	const handleSync = () => {
		...
		axios({ method: "POST", url: Routing.generate(URL_SYNC), data: {} })
			.then((response) => {
				...
				Toastr.toast('info', "Synchronisation Crypto.com Exchange terminée.");
				fetchStatus();
				onSynced && onSynced();
			})
			.catch((error) => Formulaire.displayErrors(null, error))
	...
	const handleDisconnect = () => {
		...
		axios({ method: "DELETE", url: Routing.generate(URL_DISCONNECT), data: {} })
			.then(() => {
				Toastr.toast('info', "Compte Crypto.com Exchange déconnecté.");
	...
	return <Card>
		<CardContent className="flex flex-col gap-3 p-4">
			<div>
				<div className="text-sm font-medium">Synchronisation Crypto.com Exchange (clé API)</div>
				<div className="text-xs text-muted-foreground">
					Connecte une clé API Crypto.com Exchange en lecture seule ("Can Read") pour récupérer tes trades,
					dépôts et retraits sans passer par un export manuel. Les transactions déjà importées ne sont
					jamais dupliquées.
				</div>
			</div>
		...
		<Modal ref={disconnectModal} identifiant="cryptocom-disconnect" title="Déconnecter Crypto.com Exchange"
			   content={<p>Le compte Crypto.com Exchange sera déconnecté. Les transactions déjà importées restent en place, mais tu devras reconnecter une clé API pour resynchroniser.</p>}
