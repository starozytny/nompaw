...
const URL_STATUS = "intern_api_cryptos_bitpanda_status";
const URL_CONNECT = "intern_api_cryptos_bitpanda_connect";
const URL_SYNC = "intern_api_cryptos_bitpanda_sync";
const URL_DISCONNECT = "intern_api_cryptos_bitpanda_disconnect";
...
export function BitpandaConnect ({ onSynced }) {
	...
	const fetchStatus = () => {
		axios.get(Routing.generate(URL_STATUS))
			...
			.catch((error) => Formulaire.displayErrors(null, error, "Impossible de récupérer le statut Bitpanda."))
		;
	}
	...
	const handleConnect = () => {
		...
		setConnecting(true);
		axios({ method: "POST", url: Routing.generate(URL_CONNECT), data: { apiKey } })
			.then(() => {
				Toastr.toast('info', "Compte Bitpanda connecté.");
				setApiKey("");
				fetchStatus();
			})
	...
	const handleSync = () => {
		...
		axios({ method: "POST", url: Routing.generate(URL_SYNC), data: {} })
			.then((response) => {
				...
				Toastr.toast('info', "Synchronisation Bitpanda terminée.");
				fetchStatus();
				onSynced && onSynced();
			})
			.catch((error) => Formulaire.displayErrors(null, error))
	...
	const handleDisconnect = () => {
		...
		axios({ method: "DELETE", url: Routing.generate(URL_DISCONNECT), data: {} })
			.then(() => {
				Toastr.toast('info', "Compte Bitpanda déconnecté.");
	...
	return <Card>
		<CardContent className="flex flex-col gap-3 p-4">
			<div>
				<div className="text-sm font-medium">Synchronisation Bitpanda (clé API)</div>
				<div className="text-xs text-muted-foreground">
					Connecte une clé API Bitpanda en lecture seule ("trading", "transaction", "solde") pour récupérer tes transactions
			...
			{!status.connected
				? <div className="flex flex-col gap-2">
					<input value={apiKey} onChange={(e) => setApiKey(e.target.value)} type="password"
						   placeholder="Clé API Bitpanda" className={inputClassName} />
				...
				: <div className="flex flex-col gap-2">
					<div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
						<span className="truncate max-w-[280px]">{status.apiKeyPreview}</span>
		...
		<Modal ref={disconnectModal} identifiant="bitpanda-disconnect" title="Déconnecter Bitpanda"
			   content={<p>Le compte Bitpanda sera déconnecté. Tu devras reconnecter une clé API pour resynchroniser.</p>}
